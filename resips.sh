...
# contains any entry from $BLACKLIST (default /etc/fw.black/blocklist.cfg).
# Matching is literal substring (case ... *...*), not regex, so dots in
# domains are safe. Env vars may override all three paths (used by fw-black.sh).
#
...
BLACKLIST="${BLACKLIST:-/etc/fw.black/blocklist.cfg}"
